...
class ModernTCN(nn.Module):
    ...
    def forward(self, x, te=None):
        ...
        features.append(x) # torch.Size([32, 321, 64, 180])
        x = self.head(x)
        features.append(x)
        # de-instance norm
        if self.revin:
...
class Model(nn.Module):
    ...
    def forward(self, x, x_mark_en=None, x_dec=None, x_mark_dec=None, mask=None, te=None):
        ...
        if self.decomposition:
            ...
            if te is not None:
                te = te.permute(0, 2, 1)
            res, features_res = self.model_res(res_init, te)
            trend, features_trend = self.model_trend(trend_init, te)
            features = features_res + features_trend
            x = res + trend
            x = x.permute(0, 2, 1)
        else:
            x = x.permute(0, 2, 1)
